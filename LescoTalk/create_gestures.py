...
def store_in_db(g_id, g_name):
	...
	try:
		conn.execute(cmd)
		print("Gesture saved in Database")
...
def store_images(g_id):
	...
	while True:
		...
		blur = cv2.cvtColor(imgCrop, cv2.COLOR_BGR2GRAY)
		blur = cv2.GaussianBlur(blur.copy(), (11,11), 0)
		thresh = cv2.threshold(blur,91,255,cv2.THRESH_BINARY_INV|cv2.THRESH_OTSU)[1]
